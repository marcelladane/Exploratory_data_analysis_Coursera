---
title: "First_plot"
author: "Marcelladane"
date: "26 July 2017"
output: html_document
---
**Instructions**

*Fine particulate matter (PM2.5) is an ambient air pollutant for which there is strong evidence that it is harmful to human health. In the United States, the Environmental Protection Agency (EPA) is tasked with setting national ambient air quality standards for fine PM and for tracking the emissions of this pollutant into the atmosphere. Approximatly every 3 years, the EPA releases its database on emissions of PM2.5. This database is known as the National Emissions Inventory (NEI). You can read more information about the NEI at the EPA National Emissions Inventory web site.*

For each year and for each type of PM source, the NEI records how many tons of PM2.5 were emitted from that source over the course of the entire year. The data that you will use for this assignment are for 1999, 2002, 2005, and 2008.

**PM2.5 Emissions Data structure**

The files contain the follow columns
*fips*: A five-digit number (represented as a string) indicating the U.S. county
*SCC*: The name of the source as indicated by a digit string (see source code classification table)
*Pollutant*: A string indicating the pollutant
*Emissions*: Amount of PM2.5 emitted, in tons
*type*: The type of source (point, non-point, on-road, or non-road)
*year*: The year of emissions recorded

**Question**

1 - Have total emissions from PM2.5 decreased in the United States from 1999 to 2008? Using the base plotting system, make a plot showing the total PM2.5 emission from all sources for each of the years 1999, 2002, 2005, and 2008.

First you shall loaded some packages I might need to use
Second, downloaded the data set (I called exdata) and saved on my working directory and then read 2 different tables for the SSC and NEI;
SSC = Source Classification Code
NEI = National Emissions Inventory
```{r}

packages <- c("data.table", "reshape2")
sapply(packages, require, character.only = TRUE, quietly = TRUE)

unzip("exdata.zip")

NEI <- readRDS("summarySCC_PM25.rds")
SCC <- readRDS("Source_Classification_Code.rds")
```

Now you can create a table with 2 columns, one for year and one for sum of emissions.
Since the request was for total emission per year.
Use *aggregate* to create the table and *by* to make the a sum per year.
```{r}
data <- with(NEI, aggregate(Emissions, by = list(year), sum))
head(data)
```

Finally plot the output.
No specs of the type of graphic we were suppose to makeh
I made a basic plot with green dots to show the valeus of sum and connected them using type="o"
Also added labels for the plot
```{r}
plot(data, main = "PM2.5 emissions - Total", xlab = "Year", ylab = "PM2.5 Emissions", pch = 20, col = "green", lty = 4, type="o")
```

